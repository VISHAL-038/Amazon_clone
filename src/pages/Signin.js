...
const Signin = () => {
  ...
  const [successMsg, setSucessMag] = useState("");
    ...
    if (email && password) {
      ...
      signInWithEmailAndPassword(auth, email, password)
        .then((userCredential) => {
          ...
          // ...
          setLoading(false);
          setSucessMag("Loged in Successfully!");
          setTimeout(() => {
            navigate("/");
          }, 2000);
        })
        .catch((error) => {
          ...
          if (errorCode.includes("auth/wrong-password")) {
            setUserEmailErr("Wrong Password");
          }
          console.log("Something is up, Try with correct Credetial");
        });
      setEmail("");
      setPassword("");
    }
    ...
    <div className="w-full">
      <div className="w-full bg-gray-100 pb-10">
        {successMsg ? (
          <div>
            <div>
              <p className="text-base font-titleFont font-semibold text-green-500 border-[1px] border-green-500 px-2 text-center">
                {successMsg}
              </p>
            </div>
          </div>
