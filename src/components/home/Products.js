        ...
        <div key={item.id} className="bg-white h-auto border-[1px] border-gray-200 py-6 z-30 hover:border-transparent shadow-none hover:shadow-testShadow duration-200 flex flex-col gap-4 relative ">
          ...
          <div className="w-full h-auto flex items-center justify-center relative group">
            ...
            <ul className="w-full h-36 bg-gray-100 absolute  bottom-[-170px] flex flex-col items-end justify-center gap-2 font-titleFont px-2 border-1 border-r group-hover:bottom-0 duration-500">
                ...
                <li className="productLi"  onClick={()=>dispatch(addToCart({
                  id:item.id,
                  title:item.title,
                  ...
                  image: item.image,
                  quantity:1,
                }))}>Add to Cart {""}<span><ShoppingCartIcon/></span></li>
                <li className="productLi" onClick={" "} >View Details{""}<span><ArrowCircleRightIcon/></span></li>
                <li className="productLi" onClick={()=>dispatch(addToWishList({
                  id:item.id,
                  title:item.title,
                  description:item.description,
                  price:item.price,
                  category:item.category,
                  image: item.image,
                }))}>Add to Wish List {""}<span><FavoriteIcon/></span></li>
            </ul>
          </div>
